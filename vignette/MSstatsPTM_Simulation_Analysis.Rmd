...
models %>% mutate(Model = factor(Model, levels=c("MSstatsPTM", "MSstats", "Limma", 
  ...
  labs(title = "Simulation 1: eFDR by Model", x = "Model", y = "eFDR") + 
...
models %>% mutate(Model = factor(Model, levels=c("MSstatsPTM", "MSstats", "Limma", 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust=1, size = 16), 
         ...
         title = element_text(size = 22),
         strip.text = element_text(size = 16)) + 
  labs(title = "Simulation 1: eFDR adjusted models", x = "Model", y = "eFDR") + 
   guides(color = guide_legend(order = 1, override.aes = list(size=5)), 
              size = guide_legend(order = 2, reverse = TRUE))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
png("../../MSstatsPTM-manuscript/main/images/sim3_Accuracy.png", width = 750, height = 500)
models %>% mutate(Model = factor(Model, levels=c("MSstatsPTM", "MSstatsPTM_no_adj", "limma_no_adj", 
                                               "Limma", "anova_no_adj", "ANOVA")),
                  Replicates = as.factor(rep)) %>%
  filter(Model %in% c("MSstatsPTM", "ANOVA", "Limma")) %>% 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust=1, size = 16), 
         ...
         title = element_text(size = 22),
         strip.text = element_text(size = 16)) + 
  labs(title = "Simulation 2: Accuracy of models with adjustment", x = "Model", y = "Accuracy") + 
   guides(color = guide_legend(order = 1, override.aes = list(size=5)), 
              size = guide_legend(order = 2, reverse = TRUE))
dev.off()
```
